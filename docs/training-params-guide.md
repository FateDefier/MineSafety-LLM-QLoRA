> **注意**：本文来自大佬 [code秘密花园](https://www.bilibili.com/video/BV1djgRzxEts/?spm_id_from=333.1007.top_right_bar_window_custom_collection.content.click&vd_source=8e1aeae9cf40ce80fc9b6afa0ca069ed)，**我只做了少量更改**，是一份学习笔记，供大家在调参时有一定根据，记录了参数选择的思考过程和经验总结。文中使用的示例参数（rank=8, lr=5e-5, cutoff=4096）来自早期实验，与本项目最终采用的参数（rank=16, lr=2e-4, cutoff=2048）有所不同。具体参数对比和选择原因请参考项目 README 的”训练配置”章节。
...
|**分类**|**说明**|
|---|---|
|**核心概念**|**学习率（Learning Rate）**<br><br> 决定了模型在每次更新时参数调整的幅度，通常在 (0, 1) 之间。也就是告诉模型在训练过程中 “学习” 的速度有多快。**学习率越大，模型每次调整的幅度就越大，参数在最优点附近来回震荡；学习率越小，调整的幅度就越小。**|
...
|**分类**|**说明**|
|---|---|
|**核心概念**|**训练轮数（Number of Epochs）**<br><br> Epoch 是机器学习中用于描述模型训练过程的一个术语，指的是**模型完整地遍历一次整个训练数据集的次数**。换句话说，一个 Epoch 表示模型已经看到了所有训练样本一次。**Epoch 太小，学习到的东西不足，性能不佳，欠拟合，Epoch 太大，学得太深了，容易过拟合。**|
...
|**分类**|**说明**|
|---|---|
|**核心概念**|批量大小（Batch Size） 是指在模型训练过程中，**每次更新模型参数时所使用的样本数量**。它是训练数据被分割成的小块，模型每次处理一个小块的数据来更新参数。**Batch size 过大，训练更稳定，易收敛到全局最优，但对显存消耗大，泛化能力差，易过拟合；Batch size 过小，训练不稳定且速度慢，易陷入局部最优。**|
...
|**分类**|**说明**|
|---|---|
|**核心概念**|截断长度（Max Length）决定了**模型处理文本时能接收的最大 token 数量**（token 是文本分块后的单元，如词语、子词）。它**直接影响模型对上下文信息的捕获能力，同时制约计算资源的消耗。**|
...
```python
torchrun scripts/stat_utils/length_cdf.py
    --nproc_per_node=1          
...
|**分类**|**说明**|
|---|---|
|**核心概念**|LoRA（低秩适应）中的**秩（Rank）是决定模型微调时参数更新 “表达能力” 的关键参数**。它通过低秩矩阵分解的方式，控制可训练参数的规模与模型调整的灵活程度。**秩的数值越小，模型微调时的参数更新越 “保守”，可训练参数规模小，模型调整灵活程度低，节省显存；秩的数值越大，模型能捕捉的特征复杂度越高，可训练参数规模大，模型调整灵活程度高，耗显存且易过拟合。**|
...
|**分类**|**说明**|
|---|---|
|**核心概念**|验证集比例是指**从原始训练数据中划分出用于模型性能评估的子集比例**（通常用 0-1 之间的小数表示）。这部分数据不参与模型训练，仅**用于在训练过程中实时监控模型的泛化能力，避免过拟合**。例如：验证集比例设为 0.2，表示将 20% 的数据作为验证集，80% 作为训练集。|
